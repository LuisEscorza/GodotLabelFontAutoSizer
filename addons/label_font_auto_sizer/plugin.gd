...
func _on_child_entered_tree(node: Node) -> void:
	node.child_entered_tree.connect(_on_child_entered_tree.bind()) ##Keeps checking for nested children, looking for labels.
...
func _check_nodes_in_scene() -> void:
	...
	while nodes_to_check.size() > 0:
		...
		if current_node == _current_root: ## It's either this or check if the signals are connected every single time.
			continue
